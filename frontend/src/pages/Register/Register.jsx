...
const Register = () => {
    const { register, watch, handleSubmit, formState: { error, isSubmitting },reset } = useForm();

    const handleRegisterSubmit = async (data) => {
        const response =await apiUtils.post('/auth/register',data);
                        ...
                        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                            ...
                            <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                    ...
                                    <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(handleRegisterSubmit)} >
                                        <div>
                                            <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Full Name</label>
                                            <input type="name" {...register('name', { required: "name is required" })} className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="suhani" />
                                        </div>
                                        <div>
                                        ...
                                        <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                            Alredy have an account? <Link to="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Log In</Link>
                                        </p>
                                    </form>
                                </div>
                            </div>
